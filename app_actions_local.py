...
# ------------------------------------------------------------------
# Action Rhythm (local assets) - Wake / Walk / Eat / Sleep rhythm game
# Lv15–18: 8 actions, Lv19–20: 9 actions, Reverse boss, cap Lv20.
# ------------------------------------------------------------------
...
if "init" not in st.session_state:
    st.session_state.update(
        init=True, level=1, diff=0.0, stage="start",
        seq=[], guess=[], lives=LIVES_MAX, round=0,
        best_level=1, today_level=0, today=today
    )
if st.session_state.today != today:
    st.session_state.today, st.session_state.today_level = today, 0

uid = lambda: uuid.uuid4().hex
stars = lambda lv: ("★"*max(1, lv//5)).ljust(5,"☆")

def seq_len(lv): return 9 if lv>=19 else 8 if lv>=15 else min(7, 2+lv)
def play_speed(): return max(MIN_SPEED, BASE_SPEED - 0.1*(st.session_state.level-1) - 0.2*st.session_state.diff)
...
def reset(up=False):
    ...
    if up and st.session_state.level<MAX_LEVEL: st.session_state.level+=1

def local_audio(path):
    if os.path.isfile(path):
        st.audio(f'data:audio/wav;base64,'+base64.b64encode(open(path,'rb').read()).decode(), format='audio/wav')
...
with st.sidebar:
    st.markdown('### 📅 Today'); st.write(f'Best: {st.session_state.today_level}'); st.write(stars(st.session_state.today_level))
    st.markdown('### 🏆 All‑time'); st.write(f'Best: {st.session_state.best_level}'); st.write(stars(st.session_state.best_level))
...
if st.session_state.stage=='start':
    ...
    if is_boss(): st.info('🎵 Reverse order!')
    ph=st.empty()
    for act in st.session_state.seq:
        ph.image(ACTIONS[act], width=200); local_audio(CLICK_SOUND); time.sleep(play_speed())
    ph.empty(); st.session_state.stage='guess'; st.experimental_rerun()
    ...
    for col, act in zip(cols, ACTIONS):
        with col:
            ...
            if st.button('', key=uid()):
                st.session_state.guess.append(act); local_audio(CLICK_SOUND)
    ...
    if success:
        st.success('✅ Round Clear!'); st.session_state.round+=1
    else:
        st.error('❌ Miss! Correct: '+' → '.join(target)); st.session_state.lives-=1
    if st.session_state.lives==0:
        st.error(f'Game Over – Lv {st.session_state.level}')
        if st.button('Restart', key=uid()): reset(); st.experimental_rerun()
    elif st.session_state.round==ROUNDS_PER_LEVEL and success:
        local_audio(LEVELUP_SOUND); st.balloons()
        ...
        if st.session_state.level==MAX_LEVEL:
            ...
            if st.button('Next Level', key=uid()): reset(up=True); st.experimental_rerun()
    else:
        if st.button('Next Round', key=uid()): st.session_state.update(guess=[], stage='show'); st.experimental_rerun()
